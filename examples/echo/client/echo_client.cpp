...
int ClientTcpHandler::do_message(const char *message, uint_t len)
{
    ...
    if (app.recv_count_ == app.send_count_) {
        ...
        int64_t tack_time_ms = OSApi::timestamp_ms() - app.send_start_time_;
        if (tack_time_ms <= 0) {
            tack_time_ms = 1;
        }
        ...
        printf("client connect_count:%u, recv_count:%u tack_time:%lld ms tps:%lld tps/s throughput:%0.3f MB/s\n",
            app.connect_count_,
            app.recv_count_, 
            tack_time_ms, 
            tps, 
...
int TestClientApp::send_proc(void *arg)
{
    ...
    app->send_start_time_ = OSApi::timestamp_ms();
    for (zrsocket::uint_t i = 0; i < app->connect_count_; ++i) {
        ...
        for (zrsocket::uint_t j = 0; j < client_send_count; ++j) {
            msg.head.sequence = j;
            msg.head.source = i;
            client->handler()->send((const char*)&msg, msg.head.length, false);
        }
    }
...
int main(int argc, char *argv[])
{
    if (argc < 6)
    {
        printf("please use the format: server_ip port connect_count send_count message_size (e.g.: 127.0.0.1 6611 1000 10000000 12)\n");
        return 0;
    }
