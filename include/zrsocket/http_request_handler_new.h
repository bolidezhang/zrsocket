...
protected:
    ...
    int decode(const char *data, uint_t len)
    {
        ...
        PARSE_BODY:
                uint_t remain_len = static_cast<uint_t>(data_end - p);
                if (remain_len > 0) {
                    if (request.body_.empty() && remain_len >= request.content_length_) {
                        request.body_ptr_ = const_cast<char *>(p);
                        p += request.content_length_;
                        if (decode_reset() < 0) {
                            return -1;
                        }
                    }
                        ...
                        request.body_.write(p, need_len);
                        p += need_len;
